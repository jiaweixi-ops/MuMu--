...
> **状态：EXPERIMENTAL / 未经真机验证。** `city/`、`layout/` 与 `vision/map_*` 属于后续城市规划能力，
> 当前只完成软件接口、算法与 fail-closed 验证框架。它们没有通过 Phase -1b 的真实 MuMu 模板、
> 仿射标定、建筑 catalog 或施工手势验证，也不计入 V0 四小时 Gate 的完成条件。主线优先级仍是
> **V0 真机识别 → 收取/生产闭环 → 4 小时验收**；在真实样本到位前，不继续扩展本实验栈功能。
...
这些模块当前仍是 **experimental**，其“软件测试通过”不等价于“真机可用”。在 Phase -1b 完成前，
它们不进入 V0 的验收口径，也不应继续吸收新的规划功能需求。
